...
int32_t                 VeluePrev = 0;    
#define INDIRECTPARAM_SIZE      112                             //28 Param Blocks of 4 bytes
...
void DSP_Parsing_Data(uint8_t* data)
{
  ...
  DSP_Parsing_Input_Level_Test(data, &addr, PRESET_PARAM.InputSource);
}
...
void DSP_Parsing_Input_Level(uint8_t* data, uint8_t* addr, InputSourceType* input_level)
{
  ...
  for(i = 0; i < 4; i++)
  {
    ...
    *addr +=1;
    ...
    if(level == 0) input_level[i].AnalogLevel = MIN_LEVEL_METER;
    else input_level[i].AnalogLevel = (int16_t)(((log10((double)level / 0x00800000) *10) +LEVELING_INPUT_dB) *100);
  }
  ...
  for(i = 0; i < 4; i++)
  {
    ...
    *addr +=1;
    ...
    if(level == 0) input_level[i].AesLevel = MIN_LEVEL_METER;
    else input_level[i].AesLevel = (int16_t)(log10((double)level / 0x00800000));
  }
}

void DSP_Parsing_Input_Level_Test(uint8_t* data, uint8_t* addr, InputSourceType* input_level)
{
  uint32_t level;
  uint8_t i;
  ...
  for(i = 0; i < OUT_CH_QTY; i++)
  {
    ...
    *addr +=1;
    ...
    if(level == 0) input_level[i].InputLevel = MIN_LEVEL_METER;
    else input_level[i].InputLevel = (int16_t)(((log10((double)level / 0x01000000) *20) +LEVELING_INPUT_dB) *100);
  }
...
void DSP_Parsing_RMS_Limiter_Reduction(uint8_t* data, uint8_t* addr, SpeakerOutType* out)
{
  ...
  for(i = 0; i < 4; i++)
  {
    ...
    *addr +=1;
    ...
    leveldB = log10((double)level / 0x01000000) * 20;
    
    if(leveldB >= 0) out[i].RMSLimiter.Reduction = 0;
    else out[i].RMSLimiter.Reduction = abs((int)leveldB * 100);
  }
}
...
void DSP_Parsing_Peak_Limiter_Reduction(uint8_t* data, uint8_t* addr, SpeakerOutType* out)
{
  ...
  for(i = 0; i < 4; i++)
  {
    ...
    *addr +=1;
    ...
    if(leveldB >= 0) out[i].PeakLimiter.Reduction = 0;
    else out[i].PeakLimiter.Reduction = abs((int)leveldB * 100);
...
void Led_Limiters(LEDsIndicatorsType* LED, SpeakerOutType* out)
{
  ...
  for(i = 0; i < OUT_CH_QTY; i++)
  {
    if((out[i].RMSLimiter.Reduction > THRESHOLD_REDUCTION_METER) | (out[i].PeakLimiter.Reduction > THRESHOLD_REDUCTION_METER) | (out[i].ClipLimiter.Reduction > THRESHOLD_REDUCTION_METER)) LED->LED_CLIP[i] = LED_ON;
